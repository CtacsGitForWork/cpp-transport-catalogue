...
Работает с `JSON`-запросами. Выдаёт ответ на запрос отрисовки маршрутов строкой `SVG` формата и информацию о быстрейшем пути. Реализован конструктор `JSON` с использованием цепочки вызовов методов, явные ошибки находятся на этапе компиляции.
## Описание
